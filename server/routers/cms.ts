      ...
      if (existing.length > 0) {
        await db.update(exerciseOverrides)
          .set({
            ...
            sets:       input.sets       ?? existing[0].sets,
            reps:       input.reps       ?? existing[0].reps,
            rest:       input.rest       ?? existing[0].rest,
            notes:      input.notes      ?? existing[0].notes,
            notesAr:    input.notesAr    ?? existing[0].notesAr,
