...
## Admin Content Management (CMS) — Full Control (May 22)
- [x] Create DB tables: custom_session_icons + custom_exercise_overrides
- [x] Build tRPC procedures for session icon upload/management (admin-only)
- [x] Build tRPC procedures for exercise content management (image, YouTube, name, sets, reps, rest, notes)
- [x] Build admin CMS page UI — Workout Category Icons section
- [x] Build admin CMS page UI — Exercise Manager section (image + YouTube + details editing)
- [x] Integrate custom overrides into the main app (session icons + exercise data)
- [x] Add route and navigation to CMS page from admin panel
- [x] Write tests for the new tRPC procedures

## Full Admin CMS — No-Code Control Panel (May 22)
- [x] Create DB table: site_appearance (colors, fonts, logo, banner, footer)
- [x] Create DB table: exercise_overrides (per-exercise image, YouTube, name, sets, reps, rest, notes)
- [x] Create DB table: session_icon_overrides (per-session-type custom icon)
- [x] Create DB table: app_error_logs (client errors, server errors, timestamps, user info)
- [x] Build tRPC: appearance.get / appearance.update (admin-only)
- [x] Build tRPC: appearance.uploadImage (logo, banner — admin-only, S3)
- [x] Build tRPC: exerciseOverrides.list / upsert / uploadImage (admin-only)
- [x] Build tRPC: sessionIcons.list / upsert / uploadImage (admin-only)
- [x] Build tRPC: errorLogs.list / markResolved / clear (admin-only)
- [x] Build tRPC: errorLogs.report (public — called by frontend on error)
- [x] Add "CMS" tab to AdminPanel with 3 sub-tabs: Content / Appearance / Errors
- [x] CMS > Content tab: Workout Icons grid (upload/replace per session type)
- [x] CMS > Content tab: Exercise Manager list (image upload, YouTube URL, name/sets/reps/rest/notes edit)
- [x] CMS > Appearance tab: Color pickers for primary, accent, background, text colors
- [x] CMS > Appearance tab: Font selector (curated list of Google Fonts)
- [x] CMS > Appearance tab: Logo upload + Banner upload + Footer text/links editor
- [x] CMS > Errors tab: Live error log table with severity, message, user, timestamp, resolve button
- [x] CMS > Errors tab: Unread error badge on tab when new errors arrive
- [x] Integrate appearance settings into app (CSS variables loaded from DB on startup)
- [x] Integrate exercise overrides into exerciseData (merge DB overrides at runtime)
